...
const TestimonialsSection = () => {
  ...
  <div className="text-center mb-4">
          <h2 className="text-3xl font-bold text-gray-700 mb-2">{t("testimonialsTitle")}</h2>
          <p className="text-xl text-muted">{t("testimonialsSubtitle")}</p>
        </div>
  <div className="grid md:grid-cols-3 gap-2">
          {testimonials.map((testimonial) => (
            <div key={testimonial.name} className="bg-white rounded-xl shadow-md p-4">
              <div className="flex mb-2">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <span key={i} className="text-yellow-400">★</span>
                ))}
              </div>
              <p className="text-muted mb-2 italic">"{isAr ? testimonial.quote_ar : testimonial.quote}"</p>
              <div>
